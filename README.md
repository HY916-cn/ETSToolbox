...
- 设置作业记录中的提交用时。
- 在独立终端中显示各类页面中能够识别的当前题目参考答案；实测页面未直接提供答案时，会匹配本机已经下载的题目元数据。
...
![ETSToolbox 1.0.5 功能面板](assets/settings-1.0.5.png)

### 练习中的答案终端

![练习过程中显示参考答案](assets/answer-console-1.0.5.png)

### 控分结果

![按百分比和随机偏移生成的分数](assets/score-result-1.0.5.png)

## 适用范围

控分、作业提交用时和自动流程只在已识别的听说练习页面生效。答案抓取不限制题目类型；任何页面只要提供了能够识别的参考答案数据，就会输出到答案终端。
...
- 答案提取覆盖选择、填空、口语参考表达和常见嵌套答案字段。实测 iframe 只下发题干时，前端会提取题目特征，由仅监听 `127.0.0.1` 的本地服务在 `%APPDATA%\ETS` 中匹配对应的 `content.json`/`content2.json`，再交回同一提取器处理。客户端更新内部数据结构后可能需要重新适配。
