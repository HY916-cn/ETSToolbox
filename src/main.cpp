...
namespace handlers {
	...
	void CEF_CALLBACK onLoadStart(struct _cef_load_handler_t* self,
		...
		if (frame->is_main(frame)) {
			...
			}else Logger::error("index.js文件不存在：{}。请重新运行完整发行包中的 install.ps1 安装前端资源。", STATIC_FILES_PATH);
		}
		else {
			...
			else Logger::error("index.iframe.js文件不存在：{}。请重新运行完整发行包中的 install.ps1 安装前端资源。", STATIC_FILES_PATH);
		}
